...
public class GameEngine {
	...
	public void checkStart() //Finds Start Points in an array and saves start Points 
	{
		...
		for(int y=0;y<GameMap.length;y++)
		{
			for(int x=0;x<GameMap[0].length;x++)
			{
				if(GameMap[y][x]=='S') 
				{					
					Start[0][StartCount]=y; Start[1][StartCount]=x; //Start[][], saves the position of Start Points!
					StartCount--;
	...
	public void makeRoad()
	{
		...
		{
			int y =Start[0][z]; int x= Start[1][z];			
			int step=0;
			char sign;
			boolean stop=false; 
			...
			{			
				...
				case '<': x--; break;
				case '^': y--; break;
				case 'X': stop=true; break;	//If 'X' found stop road		
				default: 
				}
				step++;
			}while(stop==false);
	...
	public void NewWave()
	{
		...
		int life; int speed; int profit; Wave++;
		
		if(Wave<11)
		{
		switch(Wave) 
		{
		case 1: life=1; speed=8; profit=10; break;
		case 2: life=2; speed=8; profit=14; break;
		case 3: life=4; speed=6; profit=16; break;
		case 4: life=6; speed=6; profit=18; break;
		case 5: life=12; speed=6; profit=24; break;
		case 6: life=10; speed=4; profit=22; break;
		case 7: life=12; speed=4; profit=24; break;
		case 8: life=14; speed=2; profit=26; break;
		case 9: life=20; speed=2; profit=100; break;
		case 10: life=100; speed=2; profit=100; break;
		default: life=1; speed=9; profit=1; break;
		}
		//Level Anpassung
		life=life*Level; if(Level==2)speed=speed/Level;
	...
	public void moveAllMonster() //Moves all GuiObjects Monsters from the current position
	{
		for(int n=NumbMonster; n>0; n--)
		{
			...
			monsterMap.get(monsterX).steps++;
				...
				switch(p)
				{				
				case 'v': GuiObject.GuiObjectMap.get(monsterX).translate(0, 1); break;	
									
				case '>': GuiObject.GuiObjectMap.get(monsterX).translate(1, 0); break;						
								
				case '<': GuiObject.GuiObjectMap.get(monsterX).translate(-1, 0); break;	
					
				case '^': GuiObject.GuiObjectMap.get(monsterX).translate(0, -1); break;	
					
				case 'X': MonsterFinish(monsterX); break; //Monster finished
		...
		for(int t=0; t<=counterT; t++)
		{
			...
			for(int m=1; m<=NumbMonster; m++) //Tower is able to shoot--is there a monster?
			{
				...
				{   
					...
					switch (type)
					{					
					case 'i': if(OmonsterX.slowSpeed()) shoot=false;  break;	
					
					case 'p': OmonsterX.durable=true;
						
					case 't':
					}				
					OmonsterX.updateLifepoints(-OtowerX.getDamage());
					if(OmonsterX.getLifepoints()<1) {killmonster(monsterX);}					
				}
				else
				{
	...
	{
		if(LifePoints<=0) {GuiObject.GuiObjectMap.clear(); monsterMap.clear(); return true;} else return false;
	}
    /**
